...
# def send_email(server,title,sender,password,receiver,message,files):
#     pass
...
class Email(object):
    ...
    def send(self):
        ...
        try:
            ...
            raise  ("发送邮件失败!无法连接到SMTP服务器,请检查网络以及服务器.%s",e)
        else:
            try:
                ...
                smtp_server.quit()
                print('邮件发送"{0}"成功！收件人:{1}。如果没有受到邮件'
                            '请检查垃圾箱，同时查看邮件地址是否正确'.format(self.title,self.receiver))

e = Email(server='smtp.163.com',
          sender='13661501664@163.com',
          receiver='13661501664@163.com;16602104373@163.com',
          password='yyy0909',
          message='这是一个测试报告模板',
          title='发送自动化测试报告',
